...
if __name__ == "__main__":
    gene_length_path = sys.argv[1]
    gene_info_path = sys.argv[2]
    groupby_col = sys.argv[3]
    outpath = sys.argv[4]
    sample_paths = dict((arg.split("=", maxsplit=1) for arg in sys.argv[5:]))
    ...
    gene_len = pd.read_table(
        gene_length_path, names=["gene_id", "genome_id", "nlength"], index_col="gene_id"
    ).nlength
