...
parser.add_option(
    "-p",
    "--program",
    help="Name or location of your GBLOCKS program (D: Gblocks)",
    action="store",
    type="str",
    dest="program",
    default="Gblocks",
...
parser.add_option(
    "-s",
    "--usestrict",
    help="Use 'strict' values according to the cited paper",
    action="store_true",
    dest="usestrict",
    default=False,
)
parser.add_option(
    "-r",
    "--userelaxed",
    help="Use 'relaxed' values according to the cited paper (Default parameters correspond to this because the paper claims it works better for shorter alignments)",
    action="store_true",
    dest="userelaxed",
    default=False,
)
...
parser.add_option(
    "-c",
    "--mincons",
    help="Minimum Number Of Sequences For A Conserved Position (D=9)",
    action="store",
    type="int",
    dest="b1",
    default=9,
...
parser.add_option(
    "-f",
    "--minflank",
    help="Minimum Number Of Sequences For A Flank Position (D=9)",
    action="store",
    type="int",
    dest="b2",
    default=9,
...
parser.add_option(
    "-n",
    "--maxnoncons",
    help="Maximum Number Of Contiguous Nonconserved Positions (D=10)",
    action="store",
    type="int",
    dest="b3",
    default=10,
...
parser.add_option(
    "-m",
    "--minblock",
    help="Minimum Length Of A Block (D=5)",
    action="store",
    type="int",
    dest="b4",
    default=5,
...
parser.add_option(
    "-g",
    "--gappos",
    help="Allowable gaps (D=h, up to half gaps - must be h (half), n (none), or a (any number))",
    action="store",
    type="str",
    dest="b5",
    default="h",
...
if not options.b5 in ["a", "h", "n", "any", "half", "none"]:
    sys.stderr.write(
        "ERROR: only valid entries for allowable gaps are a (any), n (none), or h (half)"
    )
if options.usestrict and options.userelaxed:
    sys.stderr.write(
        "ERROR: Cannot use both strict and relaxed settings at the same time!\n"
    )
    exit(2)
if options.usestrict or options.userelaxed:
    sys.stderr.write(
        "WARNING: usestrict or userelaxed will overwrite any other parameters you have specified on the command line\n"
    )
# For convenience
if options.b5 == "any":
    options.b5 = "a"
...
gblcmd = "%s %s -b1=%d -b2=%d -b3=%d -b4=%d -b5=%s -p=y > /dev/null" % (
    options.program,
    fname,
    b1,
    b2,
