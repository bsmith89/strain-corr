...
if __name__ == "__main__":
    ...
    sample_to_strain = pd.read_table(
        strain_sample_inpath, index_col=["sample"]
    ).squeeze()
    ...
    _strain_geno, _spgc_geno = strain_geno.data.to_pandas().align(spgc_geno.data.to_pandas(), axis=1, fill_value=0.5)
    strain_to_spgc_geno_num_positions = pd.DataFrame(sp.spatial.distance.cdist(
        _strain_geno != 0.5,
        _spgc_geno != 0.5,
        metric=lambda x, y: (x & y).sum(),
    ), index=_strain_geno.index, columns=_spgc_geno.index).astype(int)
    ...
    result = (
        ...
        .assign(num_geno_positions_compared=strain_to_spgc_geno_num_positions.stack())
    )
