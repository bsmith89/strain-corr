...
rule spikein_simulated_benchmark_isolate_to_mgen_read_tally:
    output: "{stemA}/reads/{mgen}_spikein_{genome}_seed{seed}_cov{cov}/{stemB}.gene_mapping_tally.tsv.lz4"
    input:
...
rule spikein_simulated_benchmark_isolate_to_mgen_gtpro:
    output: "{stemA}/reads/{mgen}_spikein_{genome}_seed{seed}_cov{cov}/{stemB}.gtpro_parse.tsv.bz2"
    input:
